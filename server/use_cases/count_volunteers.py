...
def count_volunteers_use_case(repo, request, shelter):
    ...
    work during those time intervals, for a given shelter and a tiven time period
    The time period is defined as request.filters
    """
    ...
    #   Insert the first Staffing object from workers to workforce, possibly dividing
    #   a Staffing object in workforce into multiple (slicing the time interval)
    #   If new Staffing objects are created in the process, they get added to 
    #   the workers list. 
    # In the comments below, S is a staffing object from the workforce list, and W is 
    # a staffing object from the workers list.
    while len(workers) > 0:
        ...
        for i, staff in enumerate(workforce):
            if worker.start_time == staff.start_time:
                ...
                found = True
                # S__W__W__S
                insert_staff1 = Staffing.from_dict({"start_time":worker.start_time,
                                                   "end_time":worker.end_time,
                                                   "count":staff.count+worker.count})
                
                insert_staff2 = Staffing.from_dict({"start_time":worker.end_time,
                ...
                workers.append(Staffing.from_dict({"start_time":worker.start_time,
                                                   "end_time":worker.end_time,
                                                   "count":staff.count+worker.count}))

                if not worker.end_time == staff.end_time:
                    workers.append(Staffing.from_dict({"start_time":staff.end_time,
                                                       "end_time":worker.end_time,
                                                       "count":worker.count}))
            if found:
                break
