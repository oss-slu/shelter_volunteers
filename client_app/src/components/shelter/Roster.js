      ...
      {filteredShifts.slice(0, 3).map((item, index) => (
        <div key={index} className="roster-item-container">
          ...
          <div className="roster-item">
            ...
            <span>
              {new Date(item.start_time).toLocaleString([], { hour: 'numeric', hour12: true })} - 
              {new Date(item.end_time).toLocaleString([], { hour: 'numeric', hour12: true })}
            </span>
          </div>
          ...
          <button
            ...
            onClick={() => props.onMoreDetailsClick(item)}
          >
            More Details
          </button>
        </div>
