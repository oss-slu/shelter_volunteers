...
describe("add and cancel shifts", () => {
  ...
  beforeEach(() => {
    startTime = setHours(setMinutes(setSeconds(setMilliseconds(new Date (), 0), 0), 0), new Date().getHours() + 1)
    endTime = setHours(setMinutes(setSeconds(setMilliseconds(new Date (), 0), 0), 0), new Date().getHours() + 2)
  ...
  test("shelter allows user to add and delete one shift", async() =>
  {
    render(<Shelters condensed={false} isSignupPage={true}/>)
    await waitFor(() => 
      expect(screen.getByText("Crystal Geyser Recovery")).toBeInTheDocument(),
      {timeout:3000}
    )
    ...
    await waitFor(() => expect(screen.getByText(formattedStartTime + " to " + formattedEndTime)).toBeInTheDocument())
